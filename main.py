...
def get_weather(longitude_latitude):
     
    r = requests.get(f"https://api.open-meteo.com/v1/forecast?latitude={longitude_latitude[0]}&longitude={longitude_latitude[1]}&current=temperature_2m")
...
async def weather(ctx, city):
    ...
    w = get_weather(cities_coords[city])

    await ctx.send(f"Температура в цельсиях: {w['current']['temperature_2m']}")
